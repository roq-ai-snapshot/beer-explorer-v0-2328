...
import { deleteBeerById } from 'apiSdk/beers';
...
function AccountViewPage() {
  ...
  const beerHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('beer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Beers
                    </Text>
                    <NextLink passHref href={`/beers/create?account_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.beer?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/beers/view/${record.id}`)} key={record.id}>
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              {hasAccess('beer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink passHref href={`/beers/edit/${record.id}`}>
                              ...
                              {hasAccess('beer', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={() => beerHandleDelete(record.id)}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
                                  icon={<FiTrash />}
